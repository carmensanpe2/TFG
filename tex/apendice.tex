...
\section{Transformaciones de Lorentz y Cuadrivectores}\label{cap:Lorentz}
...
A partir de los coeficientes anteriores y utilizando notación en cuadrivectores, se obtienen las \textit{Matrices de Dirac} $\gamma^{\mu}=(\gamma^0, \boldsymbol{\gamma^{i}})=(\gamma^0, \gamma^1, \gamma^2, \gamma^3)$:
\begin{align}
...
Las soluciones de esta ecuación tienen forma de onda plana y $\Psi$ se representa como un autovector de 4 componentes pero que puede expresarse en términos de dos autovectores de espín, conocidos como espinores de Dirac\cite{MCR}. 
\begin{align}
\begingroup 
\renewcommand*{\arraystretch}{0.8}
\setlength\arraycolsep{10pt}
\Psi \left( \boldsymbol{x}, t\right) =\begin{pmatrix} \psi_{1} \\ \psi_{2} \\ \psi_{3} \\ \psi_{4} \end{pmatrix}=\begin{pmatrix} u(\boldsymbol{\vec{p}},s) \\ v(\boldsymbol{\vec{p}},s) \end{pmatrix}; \qquad u(\boldsymbol{\vec{p}},s)=\begin{pmatrix} u_{1} \\ u_{2} \end{pmatrix} \qquad
v(\boldsymbol{\vec{p}},s)=\begin{pmatrix} v_{1} \\ v_{2}\label{eq:espinores} \end{pmatrix}
\endgroup
\end{align}
El espinor $u(\boldsymbol{\vec{p}},s)$ corresponde a la solución con valores de energía positivos, que describen partículas, mientras que el espinor $v(\boldsymbol{\vec{p}},s)$ está asociado a valores negativos de la energía, relacionados con las antipartículas \cite{Donelly}. Cada espinor presenta dos componentes, correspondientes a los dos posibles estados de la tercera componente de espín $s_z=\pm 1/2$. \cite{Bettini} Las soluciones conjugadas se definen como $\overline{u} \equiv u^{\dagger} \gamma^{0}$ y $\overline{v} \equiv v^{\dagger} \gamma^{0}$, respectivamente.

Los espinores $u(\boldsymbol{\vec{p}},s)$ y $v(\boldsymbol{\vec{p}},s)$ y sus conjugados cumplen la ecuación de Dirac \cite{Donelly}:
\begin{align*}
(\slashed{p}-m)u(\boldsymbol{\vec{p}},s) &=0 & (\slashed{p}+m)v(\boldsymbol{\vec{p}},s) &=0 \\
\overline{u}(\boldsymbol{\vec{p}},s)(\slashed{p}-m) &=0 & \overline{v}(\boldsymbol{\vec{p}},s )(\slashed{p}+m)&=0
\end{align*}
La normalización de espinores es \cite{Paschos}:
\begin{align*}
\overline{u}(\boldsymbol{\vec{p}},s)u(\boldsymbol{\vec{p}},s) &= 2m & \overline{v}(\boldsymbol{\vec{p}},s)v(\boldsymbol{\vec{p}},s) &= -2m
\end{align*}
Finalmente, también satisfacen las siguientes relaciones \cite{Paschos}:
\begin{align*}
\sum_s u(\boldsymbol{\vec{p}},s)\overline{u}(\boldsymbol{\vec{p}},s) &= (\slashed{p}+m) & \sum_s v(\boldsymbol{\vec{p}},s)\overline{v}(\boldsymbol{\vec{p}},s) &= (\slashed{p}-m)
\end{align*}
